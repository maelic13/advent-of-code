...
class Report:
    ...
    def sum_predicted_future_values(self) -> int:
        return sum(self.predict_value([int(value) for value in line.split()])
                   for line in self._report)

    def sum_predicted_past_values(self) -> int:
        return sum(self.predict_value([int(value) for value in line.split()][::-1])
                   for line in self._report)
...
def day9() -> None:
    with open("inputs/2023/day9.txt", "r") as file:
