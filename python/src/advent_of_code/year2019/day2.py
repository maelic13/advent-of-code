...
class Intcode:
    ...
    def run(self) -> None:
        if not self.memory:
            msg = "Internal program memory not specified."
...
def task2(computer: Intcode) -> int:
    values_to_test = [[x, y] for x in range(100) for y in range(100)]
    ...
    msg = "Computation of task 2 unsuccessful."
    raise RuntimeError(msg)


def advent2() -> None:
    intcode = Intcode()
    input_file = "inputs/2019/day2.txt"
