...
class FieldMap:
    map_signs: ClassVar[dict[str, tuple[np.ndarray, np.ndarray]]] = {
    ...
    def _find_loop_start(self) -> np.ndarray:
        for i, row in enumerate(self._field):
            for j, sign in enumerate(row):
                if sign == "S":
                    return np.array((i, j))
        msg = "Could not find starting position."
...
def day10() -> None:
    with open("inputs/2023/day10.txt", encoding="utf-8") as file:
