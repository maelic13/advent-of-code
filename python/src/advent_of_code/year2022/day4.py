...
def find_overlap(sectors: list[list[int]]):
    ...
    for sector in sectors:
        elements.update(sector)

    overlap_elements: list[int] = [
        element for element in elements if all(element in sector for sector in sectors)
    ]
    return sorted(overlap_elements)


def advent4() -> None:
    with open("inputs/2022/day4.txt", encoding="utf-8") as file:
