...
class ValveSystem:
    ...
    def apd(self, adjacency_matrix: np.matrix) -> np.matrix:
        ...
        if all(
            adjacency_matrix[i, j] for i in range(dimension) for j in range(dimension) if i != j
        ):
            return adjacency_matrix
        ...
        b_matrix = np.matrix([
            [
                1 if i != j and (adjacency_matrix[i, j] == 1 or z_matrix[i, j] > 0) else 0
        ...
        return np.matrix([
            [
                2 * t_matrix[i, j]
                if x_matrix[i, j] >= t_matrix[i, j] * degree[j]
                else 2 * t_matrix[i, j] - 1
                for j in range(dimension)
            ]
            for i in range(dimension)
        ])
    ...
    def find_by_name(self, name: str) -> Valve:
        for valve in self.valves:
            if valve.name == name:
                return valve
        msg = f"No valve with name {name} found."
    ...
    def multiple_path_score(
        self,
        valve: Valve,
        valves: list[list[Valve]],
        valve_distances: np.matrix,
        remaining_time: int,
    ) -> int:
        score = 0
    ...
    def optimal_path_score(self, remaining_time: int, players: int = 1) -> int:
        ...
        inputs = (
            (self.find_by_name("AA"), valves, self.valve_distances, remaining_time)
...
def advent16() -> None:
    with open("inputs/2022/day16.txt", encoding="utf-8") as file:
    ...
    for valve, connections in zip(valves, valves_connections, strict=False):
