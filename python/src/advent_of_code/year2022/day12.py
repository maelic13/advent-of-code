...
class Map:
    def __init__(
        self, grid_2d: list[list[int]], start: tuple[int, ...], goal: tuple[int, ...]
    ) -> None:
        self.map = grid_2d
        self.start = start
        self.goal = goal
    ...
    def get_neighbors(self, original_position: tuple[int, ...]) -> list[tuple[int, ...]]:
        ...
        if (
            not position[0] - 1 < 0
            and not self.map[position[0] - 1][position[1]] - self.map[position[0]][position[1]] > 1
        ):
            possible_moves.append(np.array((position[0] - 1, position[1])) - position)
        # down
        if (
            not position[0] + 1 > len(self.map) - 1
            and not self.map[position[0] + 1][position[1]] - self.map[position[0]][position[1]] > 1
        ):
            possible_moves.append(np.array((position[0] + 1, position[1])) - position)
        # left
        if (
            not position[1] - 1 < 0
            and not self.map[position[0]][position[1] - 1] - self.map[position[0]][position[1]] > 1
        ):
            possible_moves.append(np.array((position[0], position[1] - 1)) - position)
        # right
        if (
            not position[1] + 1 > len(self.map[0]) - 1
            and not self.map[position[0]][position[1] + 1] - self.map[position[0]][position[1]] > 1
        ):
            possible_moves.append(np.array((position[0], position[1] + 1)) - position)
...
def find_shortest_path(
    grid: list[list[int]], start_points: list[tuple[int, ...]], end_point: tuple[int, ...]
    ...
    with Pool() as pool:
        ...
        pool.close()
        pool.join()
    best_path, steps = min(results, key=operator.itemgetter(1))
...
def advent12() -> None:
    with open("inputs/2022/day12.txt", encoding="utf-8") as file:
