...
class Sensor:
    ...
    def guaranteed_empty(self, position: tuple[int, ...]) -> bool:
        return (
            position not in {self.beacon, self.position}
...
class AreaMap:
    ...
    @staticmethod
    def identify_empty(
        x_range: tuple[int, ...], y_coord: int, sensors: set[Sensor]
    ) -> list[tuple[int, ...]]:
    ...
    def find_empty(self, y_coord: int) -> set[tuple[int, ...]]:
        ...
        inputs = [
            (
                (interval[0] + i * batch_size, interval[0] + (i + 1) * batch_size),
                y_coord,
                self.sensors,
            )
        ...
        inputs[-1] = (
            (inputs[-1][0][0], inputs[-1][0][1] + line_length % cpu_count()),
            inputs[-1][1],
            inputs[-1][2],
        )
    ...
    def identify_unknown(
        sensor: Sensor, sensors: set[Sensor], limit: tuple[int, ...]
    ) -> list[tuple[int, ...]]:
    ...
    def find_unknown(self, limit: tuple[int, ...]) -> tuple[int, ...]:
        ...
        unknowns: set[tuple[int, ...]] = {x for r in results for x in r}
        if len(unknowns) != 1:
            msg = "Unknown position could not be found."
            raise RuntimeError(msg)
        return unknowns.pop()


def advent15() -> None:
    with open("inputs/2022/day15.txt", encoding="utf-8") as file:
    ...
    for line in lines:
        ...
        sensors.append(
            Sensor(
                position=(int(sensor_coord[0]), int(sensor_coord[1])),
