...
class Cave:
    def __init__(self, rocks: set[tuple[int, ...]], ground_level: bool = False) -> None:
        self.finished = False
        self.ground_level: int | None = max(x[1] for x in rocks) + 2 if ground_level else None
    ...
    def from_txt(cls, input_lines: list[str]) -> Cave:
        ...
        for formation_commands in commands:
            for i in range(len(formation_commands) - 1):
                rocks.update(
                    tuple(node)
                    for node in cls.nodes_in_between(
                        formation_commands[i], formation_commands[i + 1]
                    )
                )
        return Cave(rocks)
    ...
    def drop_sand(self) -> bool:
        ...
        position = (
            np.array(self.intersections[-1])
            if self.intersections
            else np.array(self.sand_entry_point)
        )
        while True:
            ...
            for step in [np.array((-1, 1)), np.array((1, 1))]:
                if (
                    tuple(position + step) in self.rocks.union(self.sand)
    ...
    def drop_down(self, position: np.ndarray) -> np.ndarray | None:
        blockers_downward = sorted(
            (
                np.array(x)
                for x in self.rocks.union(self.sand)
...
def advent14() -> None:
    with open("inputs/2022/day14.txt", encoding="utf-8") as file:
