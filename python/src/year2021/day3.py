...
class Diagnostic:
    ...
    def oxygen_rating(self) -> int:
        ...
        while len(sub_log) > 1:
            ...
            buff: list[str] = [number for number in sub_log if number[position] == gamma[position]]
            sub_log = buff
            position += 1
    ...
    def co2_rating(self) -> int:
        ...
        while len(sub_log) > 1:
            ...
            buff: list[str] = [
                number for number in sub_log if number[position] == epsilon[position]
            ]
            sub_log = buff
            position += 1
        return int(sub_log[0], base=2)
    ...
    def _calculate_gamma(log: list[str]) -> str:
        gamma = ""
        log = list(map(list, zip(*log, strict=False)))
...
def advent3() -> None:
    with open("inputs/2021/day3.txt", encoding="utf-8") as file:
        diagnostic = Diagnostic([line.strip() for line in file])
