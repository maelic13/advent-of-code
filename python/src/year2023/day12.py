...
class SpringRow:
    ...
    @staticmethod
    def _is_valid(springs: list[str], backup: list[int]) -> bool:
        current_count = 0
        count_index = 0

        for spring in [springs, "."]:
            if spring != "#" and current_count > 0:
                try:
                    if current_count != backup[count_index]:
                        return False
                except IndexError:
                    return False
                count_index += 1
    ...
    @staticmethod
    def _get_replaced(springs: list[str], attempt: tuple[str]) -> list[str]:
        to_use_index = 0
...
def day12() -> None:
    with open("inputs/2023/day12.txt", encoding="utf-8") as file:
