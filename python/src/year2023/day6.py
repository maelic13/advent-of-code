...
class Race:
    ...
    def __repr__(self) -> str:
        return (
            f"Race(time: {self.race_time}, record: {self.record_distance}, "
...
def day6() -> None:
    with open("inputs/2023/day6.txt", encoding="utf-8") as file:
        lines = file.readlines()

    # part 1
    times = [int(t) for t in lines[0].strip().split()[1:]]
    records = [int(r) for r in lines[1].strip().split()[1:]]
    races = list(map(Race, times, records, strict=False))
    print(prod(race.count_winning_charge_times() for race in races))

    # part 2
    race = Race(
        int("".join(lines[0].strip().split()[1:])), int("".join(lines[1].strip().split()[1:]))
    )
