...
class Node:
    ...
    def get_next(self, instruction: str) -> str:
        ...
        if instruction == "R":
            return self.right
        msg = f"Incorrect instruction: {instruction}"
    ...
    def follow_ghost_instructions(self, instructions: str) -> int:
        ...
        for node in current_nodes:
            ...
            while not self._network[current_node].is_ghost_end():
                current_node = self._network[current_node].get_next(
...
def day8() -> None:
    with open("inputs/2023/day8.txt", encoding="utf-8") as file:
        lines = file.readlines()

    instructions = lines[0].strip()
    network = Network()
    for line in lines[2:]:
        network.add_node(Node(*findall(r"[0-9A-Z]+", line)))
