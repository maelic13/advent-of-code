...
pub fn get_input(year: &str, day: &str, example: bool)
                 -> Result<Lines<BufReader<File>>, Box<dyn Error>> {
    let mut suffix: &str = "";
    if example { suffix = "_ex"; }
...
pub fn report_times(file_parse_time: f32, part1_time: f32, part2_time: f32) {
    fn check_time(time: f32) -> String {
        ...
        modified_time /= 60.;
        if modified_time < 60. {
            return format!("{:.0} minutes, {:.0} seconds", modified_time, modified_time * 60. % 60.);
        }

        modified_time /= 60.;
        if modified_time < 60. {
            return format!("{:.0} hours, {:.0} minutes", modified_time, modified_time * 60. % 60.);
        }
    ...
    println!("Execution time: {}.", check_time(part2_time - file_parse_time));
    println!();
    println!("Part 1 execution time: {}.", check_time(part1_time - file_parse_time));
    println!("Part 2 execution time: {}.", check_time(part2_time - part1_time));
}
